...
const filteredData = React.useMemo(() => {
    ...
    return data.filter((item: any) => {
      // 1. ค้นหาจากชื่อ (ใส่ ?. กันไว้เผื่อ detail เป็น null)
      const matchDetail = item.detail?.toLowerCase().includes(lower);

      // 2. ค้นหาจากสถานะ (Boolean)
      // กำหนดคำที่ต้องการให้ user ค้นเจอสำหรับ true และ false
      // เช่น true = "ใช้งาน", "active" | false = "ยกเลิก", "inactive"
      const statusText = item.status ? "พร้อมขาย" : "ขายแล้ว";
      const matchStatus = statusText.includes(lower);

      return matchDetail || matchStatus;
    });
  }, [filterValue, data]);
