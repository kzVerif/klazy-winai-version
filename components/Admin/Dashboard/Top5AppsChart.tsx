...
import { Top5Apps } from "@/lib/database/Dashboard";
ChartJS.register(BarElement, CategoryScale, LinearScale, Tooltip, Legend);

// Update Type ให้ตรงกับข้อมูลจริงที่เราดึงมา

export default function Top5AppsChart({ bestSeller }: { bestSeller: Top5Apps[] }) {
  // 2. สร้าง State สำหรับสลับโหมด
  const [mode, setMode] = useState<"sold" | "revenue">("sold");

  const chartData = {
    labels: bestSeller.map((item) => item.app_name),
    datasets: [
      {
      ...
      x: {
        ...
        ticks: {
          ...
          callback: function(val: any, index: number) {
            const label = bestSeller[index].app_name;
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 w-full h-full flex flex-col">
      ...
      <div className="flex flex-row justify-between items-center mb-6">
        <h2 className="text-lg font-bold text">Top 5 สินค้าแอปพรีเมี่ยมขายดี</h2>
